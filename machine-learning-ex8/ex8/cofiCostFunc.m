...
% Vectorized implementation of X and Theta gradient (without regularization)
X_grad = ((((X * Theta') .* R) - Y) * Theta);
Theta_grad = ((((X * Theta') .* R) - Y)' * X);

% Adding regularization to cost function written above
J = J + (((sum(sum(Theta .^ 2))) * lambda) / 2) + ((((sum(sum(X .^ 2))) * lambda)) / 2);

% Adding regularization to the gradient written above
